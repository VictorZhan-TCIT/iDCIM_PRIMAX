...
namespace Sttplay.MediaPlayer
{
    /// <summary>
    /// Renderer instance 
    /// The pixel format is UYVY422, so 1 texture is needed
    /// format:(4x4)
    /// U Y V Y U Y V Y
    /// U Y V Y U Y V Y
    /// U Y V Y U Y V Y
    /// U Y V Y U Y V Y
    /// </summary>
    public class SCRendererUYVY422 : SCRenderer
    {

        public override void InitRenderer(SCFrame frame)
        {
            SourceTextures = new Texture2D[2];
            RenderMaterial = new Material(Shader.Find("Sttplay/UYVY422"));
        ...
        {
            if (frame.format != (int)PixelFormat.UYVY422)
            {
