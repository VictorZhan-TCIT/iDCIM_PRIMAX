...
namespace Sttplay.MediaPlayer
{
    /// <summary>
    /// Renderer instance 
    /// The pixel format is YUV420P, so 3 textures are required
    /// format:(4x4)
    /// Y Y Y Y
    /// Y Y Y Y
    /// Y Y Y Y
    ...
    /// V V
    /// </summary>
    public class SCRendererYUV420P : SCRenderer
    {
        ...
        public override void InitRenderer(SCFrame frame)
        {
            ...
            SourceTextures[1] = new Texture2D(frame.linesize[1], frame.height / 2, GraphicsFormat.R8_UNorm, TextureCreationFlags.None);
            SourceTextures[2] = new Texture2D(frame.linesize[2], frame.height / 2, GraphicsFormat.R8_UNorm, TextureCreationFlags.None);
            base.InitRenderer(frame);
        }

        public override void Renderer(SCFrame frame)
        {
            if (frame.format != (int)PixelFormat.YUV420P && frame.format != (int)PixelFormat.YUVJ420P)
            {
