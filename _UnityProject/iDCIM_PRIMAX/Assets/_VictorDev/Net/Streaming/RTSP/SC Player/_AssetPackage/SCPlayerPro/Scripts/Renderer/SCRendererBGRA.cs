...
namespace Sttplay.MediaPlayer
{
    /// <summary>
    /// Renderer instance 
    /// The pixel format is BGRA, so 1 texture is needed
    /// format:(4x4)
    ...
    /// B G R A B G R A B G R A B G R A
    /// </summary>
    public class SCRendererBGRA : SCRenderer
    {

        public override void InitRenderer(SCFrame frame)
        {
            SourceTextures = new Texture2D[1];
            RenderMaterial = new Material(Shader.Find("Sttplay/BGRA"));
        ...
        {
            if (frame.format != (int)PixelFormat.BGRA)
            {
