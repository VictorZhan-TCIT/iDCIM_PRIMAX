...
namespace Sttplay.MediaPlayer
{
    /// <summary>
    /// Renderer instance 
    /// The pixel format is RGBA, so 1 texture is needed
    /// format:(4x4)
    ...
    /// R G B A R G B A R G B A R G B A
    /// </summary>
    public class SCRendererRGBA : SCRenderer
    {

        public override void InitRenderer(SCFrame frame)
        {
            SourceTextures = new Texture2D[1];
            RenderMaterial = new Material(Shader.Find("Sttplay/RGBA"));
        ...
        {
            if (frame.format != (int)PixelFormat.RGBA)
            {
