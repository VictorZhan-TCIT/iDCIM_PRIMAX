...
namespace Sttplay.MediaPlayer
{
    ...
    /// <summary>
    /// Renderer instance 
    /// The pixel format is BGR24, so 1 texture is needed
    /// format:(4x4)
    /// B G R B G R B G R
    /// B G R B G R B G R
    /// B G R B G R B G R
    /// B G R B G R B G R
    /// </summary>
    public class SCRendererBGR24 : SCRenderer
    {

        public override void InitRenderer(SCFrame frame)
        {
            SourceTextures = new Texture2D[1];
            RenderMaterial = new Material(Shader.Find("Sttplay/BGR"));
        ...
        {
            if (frame.format != (int)PixelFormat.BGR24)
            {
