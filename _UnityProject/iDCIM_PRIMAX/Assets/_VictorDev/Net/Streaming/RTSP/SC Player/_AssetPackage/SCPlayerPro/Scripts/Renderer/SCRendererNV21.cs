...
namespace Sttplay.MediaPlayer
{
    /// <summary>
    /// Renderer instance 
    /// The pixel format is NV21, so 2 textures are required
    /// format:(4x4)
    /// Y Y Y Y
    /// Y Y Y Y
    /// Y Y Y Y
    ...
    /// V U V U
    /// </summary>
    public class SCRendererNV21 : SCRenderer
    {

        public override void InitRenderer(SCFrame frame)
        {
            SourceTextures = new Texture2D[2];
            RenderMaterial = new Material(Shader.Find("Sttplay/NV21"));
        ...
        {
            if (frame.format != (int)PixelFormat.NV21)
            {
