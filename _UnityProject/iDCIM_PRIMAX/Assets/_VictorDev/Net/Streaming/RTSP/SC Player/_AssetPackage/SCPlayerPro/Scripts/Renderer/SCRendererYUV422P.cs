...
namespace Sttplay.MediaPlayer
{
    /// <summary>
    /// Renderer instance 
    /// The pixel format is YUV422P, so 3 textures are required
    /// format:(4x4)
    /// Y Y Y Y
    /// Y Y Y Y
    /// Y Y Y Y
    ...
    /// V V
    /// </summary>
    public class SCRendererYUV422P : SCRenderer
    {
        ...
        {
            if (frame.format != (int)PixelFormat.YUV422P && frame.format != (int)PixelFormat.YUVJ422P)
            {
